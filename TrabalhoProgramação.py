...
# lock pra proteger o estado
lock_estado = threading.Lock()

# filas usadas pelas threads
fila_sensores = Queue()
fila_log = Queue()
...
def sensor(via, intervalo):
    while rodando:
        ...
        if novos_carros > 0:
            ...
            # manda os dados pro controlador
            fila_sensores.put({"via": via, "carros": novos_carros})

            # envia mensagem pro logger
            fila_log.put(
...
# thread que controla o semáforo
def controlador(tempo_ciclo):
    ...
    while rodando:
        ...
        while not fila_sensores.empty():
            ...
            # lock pra acessar memória compartilhada
            with lock_estado:
